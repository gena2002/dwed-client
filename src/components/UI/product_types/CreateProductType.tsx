...
const CreateProductType = () => {
    ...
    function submit(e: React.FormEvent<HTMLFormElement>) {
        e.preventDefault();
        if (!store.admin_product_type.post_loader.getLoading()) {
            store.admin_product_type.post(name)
                .then(r => {
                    console.log(r)
                    if (r.status === 201) {
                        setErrors([
                        ...
                        store.alert.setAlert(true, 'success', 'Product Type is created success');
                        navigate(`/product_types/list`)
                    } else {
                        console.log(r)
    ...
    return (
        ...
        >
            ...
            }} loading={store.admin_product_type.post_loader.getLoading()}>{T('Create')}</LoadingButton>

        </Root>
    );
};

export default observer(CreateProductType);

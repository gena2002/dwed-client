...
const EditProductType = () => {
    ...
    useEffect(() => {
        if (id !== null) {
            store.admin_product_type.get(id).then(r => {
                console.log(r)
                switch (r.status) {
                    case 200:
                        setName(r?.data?.name)
    ...
    function submit(e: React.FormEvent<HTMLFormElement>) {
        e.preventDefault();
        if (!store.admin_product_type.patch_loader.getLoading() && id !== null) {
            store.admin_product_type.patch(id, name)
                .then(r => {
                    console.log(r)
                    ...
                    switch (r.status) {
                        case 200:
                            ...
                            store.alert.setAlert(true, 'success', 'Region Type is updated success');
                            break;
                        case 400:
                            console.log(r)
    ...
    if (store.admin_product_type.get_loader.getLoading()) return <div>Loading from EditRegionType</div>
    ...
    return (
        ...
        >
            <FlexRow>
                <BackButton to={'/product_types/list'}/>
                <Title24>
                    Edit Region Type
                </Title24>
            </FlexRow>
            ...
            }} loading={store.admin_product_type.post_loader.getLoading()}>{t("Save")}</LoadingButton>

        </Root>
    );
};

export default observer(EditProductType);

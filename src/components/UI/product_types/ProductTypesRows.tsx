...
function ProductTypesRows({rows, reloadRegionList, loading}: Props) {
    ...
    return (<>
            {rows.map((row) => (
                <StyledTableRow key={row.id} style={{
                    ...
                    navigate(`/product_types/list/${row.id}`)
                }}>
                    ...
                    <TableCell padding="checkbox" align="center">
                        <EditRow edit_link={`/product_types/list/${row.id}`} onDelete={() => {
                            store.admin_product_type.delete(row.id).then(() => {
...
export default observer(ProductTypesRows);

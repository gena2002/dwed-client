...
const CreateOfficialField = () => {
    ...
    function submit(e: React.FormEvent<HTMLFormElement>) {
        e.preventDefault();
        if (!store.admin_official_field.post_loader.getLoading()) {
            store.admin_official_field.post(name, true, 1)
                .then(r => {
                    console.log(r)
                    // if (r.status === 201) {
                    // } else {
                    ...
                    if (r.status === 201) {
                        ...
                        store.alert.setAlert(true, 'success', 'Official Field is created success');
                        navigate(`/official_fields/list`)
                    } else {
                        console.log(r)
    ...
    return (
        ...
        >
            ...
            }} loading={store.admin_official_field.post_loader.getLoading()}>{T('Create')}</LoadingButton>

        </Root>
    );
};

export default observer(CreateOfficialField);

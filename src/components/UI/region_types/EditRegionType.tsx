...
const EditRegionType = () => {
    ...
    useEffect(() => {
        if (id !== null) {
            store.admin_region_type.get(id).then(r => {
                console.log(r)
                switch (r.status) {
                    case 200:
                        setName(r?.data?.name)
    ...
    function submit(e: React.FormEvent<HTMLFormElement>) {
        e.preventDefault();
        if (!store.admin_region_type.patch_loader.getLoading() && id !== null) {
            store.admin_region_type.patch(id, name)
                .then(r => {
                    console.log(r)
    ...
    if (store.admin_region_type.get_loader.getLoading()) return <div>Loading from EditRegionType</div>
    ...
    return (
        ...
        >
            <FlexRow>
                <BackButton to={'/region_types/list'}/>
                <Title24>
                    Edit Region Type
                </Title24>
            </FlexRow>
            ...
            }} loading={store.admin_region_type.post_loader.getLoading()}>{t("Save")}</LoadingButton>

        </Root>
    );
};

export default observer(EditRegionType);

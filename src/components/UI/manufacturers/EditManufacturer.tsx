...
const EditManufacturer = () => {
    ...
    useEffect(() => {
        if (id !== null) {
            store.admin_manufacturer.get(id).then(r => {
                console.log(r)
                switch (r.status) {
                    case 200:
                        setName(r?.data?.name)
    ...
    function submit(e: React.FormEvent<HTMLFormElement>) {
        e.preventDefault();
        if (!store.admin_manufacturer.patch_loader.getLoading() && id !== null) {
            store.admin_manufacturer.patch(id, name, null)
                .then(r => {
                    console.log(r)
                    ...
                    switch (r.status) {
                        case 200:
                            ...
                            store.alert.setAlert(true, 'success', 'Manufacturer is updated success');
                            break;
                        case 400:
                            console.log(r)
    ...
    if (store.admin_manufacturer.get_loader.getLoading()) return <div>Loading from EditRegionType</div>
    ...
    return (
        ...
        >
            <FlexRow>
                <BackButton to={'/manufacturers/list'}/>
                <Title24>
                    Edit Region Type
                </Title24>
            </FlexRow>
            ...
            }} loading={store.admin_manufacturer.post_loader.getLoading()}>{t("Save")}</LoadingButton>

        </Root>
    );
};

export default observer(EditManufacturer);
